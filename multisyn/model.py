...
def add_attn(node, field, attn): 
    ...
    return {
        field: (attn(feat, node.mailbox['m'], node.mailbox['m']) + feat).squeeze(1)  
    }  
def clones(module, N): 
...
class MVMP(nn.Module):
    def __init__(self, msg_func=add_attn, hid_dim=300, depth=3, view='aba', suffix='apj', act=nn.ReLU()):
        """
    ...
    def forward(self, bg):
        ...
        update_funcs.update({e: (fn.copy_u(f'f_junc_{suffix}', 'm'), partial(self.msg_func, 
