...
def GetBricsBondFeature(action):
    ...
    result = []
    start_action_bond = (
        int(action[0]) if (action[0] != "7a" and action[0] != "7b") else 7
    )
...
def GetFragmentFeats(mol):
    ...
    break_bonds = [
        mol.GetBondBetweenAtoms(i[0][0], i[0][1]).GetIdx() for i in FindBRICSBonds(mol)
    ]
    if break_bonds == []:
        tmp = mol
    else:
        tmp = Chem.FragmentOnBonds(
            mol, break_bonds, addDummies=False
        )  # Cut into segments
...
def Mol2HeteroGraph(mol):
    ...
    g.nodes["a"].data["f_junc"] = torch.cat(
        [g.nodes["a"].data["f"], torch.zeros(dim_atom_padding, dim_pharm)], 1
    )
    ...
    for i in range(g.num_edges(etype=("a", "b", "a"))):
        f_bond.append(
            bond_features(mol.GetBondBetweenAtoms(src[i].item(), dst[i].item()))
        )
    ...
    for idx in range(g.num_edges(etype=("p", "r", "p"))):
        ...
        for (
            ...
            bbr
        ):  # bbr BrICS-Bond [[start atom index, end atom index], BRICS bond pair feature information]
            p0 = result_ap[i[0][0]]
            p1 = result_ap[i[0][1]]
